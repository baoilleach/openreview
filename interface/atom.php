	...
	if ($type == "search") {
		...
		if ($search) {
			...
			if (isset($return["total_results"])) {
				...
				$buffer .= atom_header("Search results for $safe_search");
...
<opensearch:Query rel=\"request\" searchTerms=\"$safe_search\" />
	...
	if ($type == "latest_links") {
		...
		$title = "Postgenomic - latest links";		
		if ($category) {
			$title = "Postgenomic - latest ".strtolower($category)." links";
	...
	if ($type == "latest_papers") {
		...
		$title = "Postgenomic - latest papers";		
		if ($category) {
			$title = "Postgenomic - latest ".strtolower($category)." papers";
			$filters['category'] = $category;
		}
	...
	if ($type == "latest_posts") {
		...
		$title = "Postgenomic - latest posts";		
		if ($category) {
			$title = "Postgenomic - latest ".strtolower($category)." posts";
			$filters['category'] = $category;
		}
